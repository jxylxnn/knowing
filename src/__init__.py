...
def _install_test_torch_shim() -> None:
    ...
    The local Torch build aborts on import in this environment. Most tests only
    ...
    torch.randn = lambda *shape, generator=None, device=None: np.random.randn(*shape).astype(np.float32)
    torch.rand = lambda *shape, generator=None, device=None: np.random.rand(*shape).astype(np.float32)
